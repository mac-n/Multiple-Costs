...
#from FSelector package
cost_cfs_2 <- function(formula, data, costs, lamda, budget,costs2,lamda2=0,budget2) {
  names(costs)<-names(data)
  names(costs2)<-names(data)
  ...
  evaluator <- function(attrs) {
    if (sum(costs[attrs])>budget){
      #print(0)
      return(0)
    }
if (sum(costs2[attrs])>budget2){
      #print(0)
      return(0)
    }
    #p,"FDrint(attrs)
    ...
    for(i in 1:attr_count) {
     ...
     fc_sum = fc_sum + cor
      ...
      cost_sum2 =cost_sum2+costs2[attr1]
    ...
    cost_mean2 =cost_sum2/attr_count
    if(attr_count == 1){
    #print(fc_cor - cost_mean*lamda)
      return(fc_cor - cost_mean*lamda -cost_mean2*lamda2)}
    else{
      #edited this line obviously
      #print(attr_count * fc_cor / sqrt(attr_count + attr_count * (attr_count - 1) * ff_cor)-(cost_mean*lamda))
      return(attr_count * fc_cor / sqrt(attr_count + attr_count * (attr_count - 1) * ff_cor)-(cost_mean*lamda)-cost_mean2*lamda2)
    }
  }
  ...
  names(costs2)<-attr_names[2:attr_count]
  entropies = NULL
  if(classification) {
  ...
  result = best.first.search(names(new_data)[-1], evaluator,max.backtracks=12)
